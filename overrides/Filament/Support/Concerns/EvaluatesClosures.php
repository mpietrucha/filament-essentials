...
trait EvaluatesClosures
{
    ...
    protected function resolveClosureDependencyForEvaluation(ReflectionParameter $parameter, array $namedInjections, array $typedInjections): mixed
    {
        ...
        if (filled($typedParameterClassName)) {
            ...
            foreach ($this->getTypedNamedInjections() as $type => $name) {
                if (
                    is_a($typedParameterClassName, $type, true) &&
                    array_key_exists($name, $namedInjections)
                ) {
                    return value($namedInjections[$name]);
                }
            }
        }
     ...
     */
    protected function getTypedNamedInjections(): array
    {
        return [
            EloquentBuilder::class => 'query',
